...
def MonthSortFileList(filesin):
  ...
  #if files != filesin:
    #JHR 8 Aug don't resort unless necessary because it fouls things up elsewhere
...
def printCollation(filelist):
  #assert MonthSortFileList(filelist) == filelist
  individuals={}
  yearset = set()
  keylist=[]
  print "years,",
...
adjusted.csv_cru.final.csv shuffleRpt $Revision: 288 $
adjusted.csv_cru.final.csv shuffle $Revision: 286 $
adjusted.csv_cru.final.csv bivariate_multi as bivariate $Revision: 297 $
adjusted.csv_cru.final.csv ConvergentBreaks as convergent_breaks $Revision: 286 $
adjusted.csv_cru.final.csv regress $Revision: 291 $
adjusted.csv_cru.final.csv statbreaks $Revision: 288 $
...
r1i1p1_FGOALS_g2_RCP2.6_icmip5_tas_Amon_ens_rcp26_0-360E_-90-90N_n_030.dat.final.csv shuffleRpt $Revision: 288 $
r1i1p1_FGOALS_g2_RCP2.6_icmip5_tas_Amon_ens_rcp26_0-360E_-90-90N_n_030.dat.final.csv shuffle $Revision: 286 $
r1i1p1_FGOALS_g2_RCP2.6_icmip5_tas_Amon_ens_rcp26_0-360E_-90-90N_n_030.dat.final.csv bivariate_multi as bivariate $Revision: 297 $
r1i1p1_FGOALS_g2_RCP2.6_icmip5_tas_Amon_ens_rcp26_0-360E_-90-90N_n_030.dat.final.csv ConvergentBreaks as convergent_breaks $Revision: 286 $
r1i1p1_FGOALS_g2_RCP2.6_icmip5_tas_Amon_ens_rcp26_0-360E_-90-90N_n_030.dat.final.csv regress $Revision: 291 $
r1i1p1_FGOALS_g2_RCP2.6_icmip5_tas_Amon_ens_rcp26_0-360E_-90-90N_n_030.dat.final.csv statbreaks $Revision: 288 $
'''
...
def parsefilename(filename):
  ...
  #"uahncdc_tp_6.0beta1_SoPolOcean.final.csv"
    ...
    if words[1][:-len("Ocean")] == "Ocean":
      ...
      rcp="LandOcean"
    rep = words[1]
  elif filename[:len("RSS_MON")] == "RSS_Mon":
    words=filename.split('_')
...
#chnages NCDC Monthly set, Llan, Ocean, LandOcean
      elif filename.find("_LandOcean-") >=0 or filename.find("-LandOcean-") >=0:
        model='NCDCLandOcean'
        posn1 = filename.find("_LandOcean-")+len("_LandOcean-")-1
        if filename.find("Jan-Dec") >0 or filename.find("JanDec") >0:
          rcp="Annual"
        else:
          rcp=filename[posn1+1:posn1+4]
        rep=filename[:2]
...
#the cmip5 resiulst downloaded with names commencing with r1i1p1 etc
      ...
      #"uahncdc_tp_6.0beta1_SoPolOcean.final.csv"
        ...
        if words[1][:-len("Ocean")] == "Ocean":
          ...
          rcp="LandOcean"
        rep = words[1]
      elif filename[:len('aravg.ann')] == 'aravg.ann':
...
def extractfnset(infile):
  with open(infile, 'r') as f:
    lines=f.readlines()
    fset = {}
    flist=[]
    for line in lines:
      fkey=line[:line.find(".final.csv")]
      if not fkey in flist:
        flist.append(fkey)
      fset[fkey] = 1
  return flist #sorted(fset.keys())
    
def uniformSort(xlist): #
  '''
  sort the input list on the file name at the start
  format looks like 'r10i1p1_CSIRO-Mk3-6-0_RCP2.6_icmip5_tas_Amon_ens_rcp26_0-360E_-90-90N_n_027.dat.final.csv "[1861.0, 1900, 1921, 2001, 2022, 2043, 2100.0]" 98 1900 1861.0 1921 100 19.8019489537 0.155298546348 100 1900 0.98 1897 0.02 0.00261321126179 -0.00215571216526 -0.00551179654818 0.257784849611 0.0229254534194 8.74341586599 0.230419594087 1900 0.83 1895 0.08 0.85\n'
  the key in not unique but we need to retain all lines
  '''
  xdict={}
  for line in xlist:
    fkey=line[:line.find(".final.csv")]
    if fkey not in xdict:
      xdict[fkey]=[]
    xdict[fkey].append(line)
  outlist=[]
  #now sort as Roger wanted with mon and case independednt
  for fkey in MonthSortFileList(xdict.keys()): #case independent and month replacing sort
    for line in xdict[fkey]:
      outlist.append(line)
  assert len(xlist) == len(outlist), "outlist not same as xlist"
  return outlist 
#  
if __name__ == "__main__":
  
  import os
  printFinal_current(['RCP26_25jun.x'], "RCP26_test_uniform2.x.csv", cmip5=True)
  printCollation(extractfnset('RCP26_25jun.x')) 

#==============================================================================
#   for xfile in ['ncnc_jim.x']:
#     if not os.path.exists(xfile+'.csv'):
#       printFinal_current([xfile], xfile+'.csv',cmip5=False)
#       printCollation(extractfnset(xfile)) 
#       sys.exit()
#==============================================================================
...
#  printCollation(glob.glob("uahncdc_*[lnd8tH9T]")) 
 #8 may 
#RSS_Monthly_MSU_AMSU_Channel_TTT_Anomalies_Ocean_v03_3.txt_82.5S-82.5N_TTT.final.csv  
#  printFinal_current(['rss_monthly.x'], "RSS_monthly.csv",cmip5=False)
#  printCollation(glob.glob("RSS_Mon*[TS]"))
  #14May Seasonal NCDC and Hadley
  #print extractfnset('NCDC_all.x')
#  printFinal_current(['NCDC_all.x'], "NCDC_seasonal.csv",cmip5=False)
#  printCollation(extractfnset('NCDC_all.x')) 
#  printFinal_current(['HADCRU_SEAS.x'], "HADCRU_seasonal.csv",cmip5=False)
#  printCollation(extractfnset('HADCRU_SEAS.x')) 
#  printFinal_current(['HADCRUT4_CRU_Repeat_20150525.x'], "HADCRUT4_CRU_Repeat_20150525.csv",cmip5=False)
#  printCollation(extractfnset('HADCRUT4_CRU_Repeat_20150525.x')) 
#26 May reruns
#  
#  printFinal_current(['ncdc_ann.x'], "ncdc_ann.csv",cmip5=False)
#  printCollation(extractfnset('ncdc_ann.x')) 
#  printFinal_current(['hadcrut_20150525.x'], "hadcrut_20150525.csv",cmip5=False)
#  printCollation(extractfnset('hadcrut_20150525.x')) 
#  printFinal_current(['HadSST.3.1.1.0_annual.x'], "HadSST.3.1.1.0_annual.csv",cmip5=False)
#  printCollation(extractfnset('HadSST.3.1.1.0_annual.x')) 
#3Jun NCDC_All_Zones.csv
#  printFinal_current(['NCDC_All_Zones.x'], "NCDC_All_Zones.csv",cmip5=False)
#  printCollation(extractfnset('NCDC_All_Zones.x')) 
#*June Annual Hadley results accidently included 2015
#  printFinal_current(['HadCRUT4_20150607.x'], "HadCRUT4_20150607.csv",cmip5=False)
 # printCollation(extractfnset('HadCRUT4_20150607.x')) 
#  printFinal_current(['HadSST_20150607.X'], "HadSST_20150607.csv",cmip5=False)
## printCollation(extractfnset('HadSST_20150607.X')) 
#  printFinal_current(['GlobaTemps.x'], "GlobaTemps.repeat.csv",cmip5=False)
#  printCollation(extractfnset('GlobaTemps.x')) 
#  printFinal_current(['cmip5_piControl.x'], "cmip5_piControl.repeat.csv",cmip5=True)
#  printCollation(extractfnset('cmip5_piControl.x')) 
#  printFinal_current(['HADCRU_SEAS.x'], "HADCRU_seasonal.repeat.csv",cmip5=False)
#  printCollation(extractfnset('HADCRU_SEAS.x')) 
#25jun omitted models
  
#  printFinal_current(['RCP60_25jun.x'], "RCP60_25jun.x.csv",cmip5=False)
#  printCollation(extractfnset('RCP60_25jun.x')) 
#  printFinal_current(['RCP85_25jun.x'], "RCP85_25jun.x.csv",cmip5=False)
#  printCollation(extractfnset('RCP85_25jun.x')) 
#  printFinal_current(['RCP45_25jun.x'], "RCP45_25jun.x.csv",cmip5=False)
#  printCollation(extractfnset('RCP45_25jun.x')) 
#  printFinal_current(['piControl_25jun.x'], "piControl_25jun.x.csv",cmip5=False)
#  printCollation(extractfnset('piControl_25jun.x')) 
  '''
