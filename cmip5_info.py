...
'''
groupmodels={'NOAA',[], #dummy name used by some groups
'ICHEC': ['EC-EARTH', 'EC-EARTH-2-2'], 
...
centres={'ICHEC':["ie1","ireland",'EC-Earth (European Earth System Model)'],
 ...
 'NOAA':["us0","USA","Composite group ??"]
 }

countries=     ['au', 'ca', 'cn', 'de', 'fr', 'ie', 'it', 'jp', 'kr','no', 'ru',  'uk', 'us']
cmip3countries=['au', 'ca', 'cn', 'de', 'fr',       'it', 'jp',      'no', 'ru',  'uk', 'us']
...
#  rip="%04d" % (100*int(r)+10*int(i)+int(p),)
  return "%04d" % (1000*int(p)+100*int(i)+int(r),)
  
def codes(model, fn):
  replicate = os.path.basename(fn).split("_")[0]
  rcp=os.path.basename(fn).split("_")[2]
  if rcp=="icmip5":
    rcp=os.path.basename(fn).split("_")[6][:3]
  ...
  r=r[0]
#  rip="%04d" % (100*int(r)+10*int(i)+int(p),)
  rip=ripcode(replicate)
    
  return grp,alphanumcode,"%03d"%(groupno,),model, "%02d"% (modelingroupnumber,), "%03d%02d"%(groupno,modelingroupnumber), replicate, rip, rcp
  
header='"ModelGroup","GroupCode","GroupNum","Model","GroupModNum","ModelNum","EnsembleCode","EnsembleNum","Scenario"'
def matchfn(fn):
  #find rcp in file name
...
if __name__ == "__main__":
  ...
  fns=glob.glob("c:\\users\\s4493222\\documents\\abrupt\\c_test\\r*_*GISS*[0123456789t]")
  print fns
  for fn in fns:
    code=matchfn(fn)
    if code != None:
